...
resource "aws_instance" "default" {
  ...
  tags = "${merge(map("Name", format("%s-%d", var.name, count.index+1)),
    var.instance_tags,
    map("private_name", var.tag_private_name),
    map("public_name" , var.tag_public_name),
    map("environment" , var.tags_environment),
    map("app"         , var.tag_app),
...
resource "aws_route53_record" "private" {
  ...
  count   = "${var.register_dns_private}"
}
